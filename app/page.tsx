    ...
    <main className="appShell">
      <aside className="sidebar">
        ...
        <nav className="mainNav" aria-label={lang === "en" ? "Main navigation" : "メインナビゲーション"}>
          ...
          <button className={view === "review" ? "active" : ""} onClick={() => setView("review")}><span>02</span>{t("確認キュー")}<b>{reviewCount}</b></button>
          <button className={view === "architecture" ? "active" : ""} onClick={() => setView("architecture")}><span>03</span>{t("接続設計")}</button>
        </nav>
      ...
      <section className="workspace">
        <header className="topbar">
          <div><span className="eyebrow">HR REVIEW CONSOLE</span><h1>{view === "overview" ? t("共有理解のズレを、案件単位で見つける。") : view === "review" ? t("今週の確認キュー") : t("読み取り専用パイロット設計")}</h1></div>
