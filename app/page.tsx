...
type View = "overview" | "pipeline" | "review" | "impact" | "architecture";
type Lang = "ja" | "en";
...
const EN: Record<string, string> = {
  ...
  "働き方の変化と、次の段階を示す。": "Show the workflow impact and next stages.",
  "CSV出力": "Export CSV",
  "照合中...": "Analyzing...",
    ...
    <main className="appShell">
      <aside className="sidebar">
        ...
        <nav className="mainNav" aria-label={lang === "en" ? "Main navigation" : "メインナビゲーション"}>
          ...
          <button className={view === "impact" ? "active" : ""} onClick={() => setView("impact")}><span>04</span>{t("効果・ロードマップ")}</button>
      ...
      <section className="workspace">
        <header className="topbar">
          <div><span className="eyebrow">HR REVIEW CONSOLE</span><h1>{view === "overview" ? t("共有理解のズレを、案件単位で見つける。") : view === "pipeline" ? t("実データから問題検知までを動かす。") : view === "review" ? t("今週の確認キュー") : view === "impact" ? t("働き方の変化と、次の段階を示す。") : t("読み取り専用パイロット設計")}</h1></div>
        ...
        {view === "impact" && <ImpactRoadmap lang={lang} />}
